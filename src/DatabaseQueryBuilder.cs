...
namespace Extension.DatabaseQueryBuilder
{
    public class DatabaseQueryBuilder
    {
        ...
        public DatabaseQueryBuilder()
        {
            ...
            var settingsFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "carbon", "extensions", "data", $"{nameof(DatabaseQueryBuilder)}", "settings.yaml");
        ...
        {
            var defaultSettings = new Settings { managing_tables = false };
            var serializer = new SerializerBuilder()
                .WithNamingConvention(CamelCaseNamingConvention.Instance)
                .Build();
        ...
        public void AddIndex(string tableName, string indexName, params string[] columns)
        {
            using var connection = getConnection();
            connection.Open();
